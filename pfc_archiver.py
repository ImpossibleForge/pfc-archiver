#!/usr/bin/env python3
"""
pfc-archiver v0.1.0 — Autonomous cold partition archive daemon
==============================================================
...
Supported databases:
  PostgreSQL wire: CrateDB (port 5432)
...
Usage:
  python pfc_archiver.py --config config/cratedb.toml
...
log = logging.getLogger("pfc-archiver")
...
# ---------------------------------------------------------------------------
# Database connector (CrateDB via PostgreSQL wire protocol)
# ---------------------------------------------------------------------------
...
def _connect(db_cfg: dict):
    ...
    return psycopg2.connect(
        ...
        user=db_cfg.get("user", "crate"),
        password=db_cfg.get("password", ""),
        dbname=db_cfg.get("dbname", "doc"),
        connect_timeout=30,
    )
...
def get_partition_ranges(db_cfg: dict, retention_days: int, partition_days: int) -> list:
    ...
    Returns list of (from_ts, to_ts) tuples — each one day (or partition_days) wide.
    ...
    full_tbl  = f'"{schema}"."{table}"'
    cutoff_ts = datetime.now(timezone.utc) - timedelta(days=retention_days)

    try:
        cur = conn.cursor()
        cur.execute(f'SELECT MIN("{ts_col}"), MAX("{ts_col}") FROM {full_tbl}')
        row = cur.fetchone()
        cur.close()
    finally:
        conn.close()
    ...
    if isinstance(min_ts, (int, float)):
        min_ts = datetime.fromtimestamp(min_ts / 1000, tz=timezone.utc)
...
def export_partition_to_pfc(
    ...
    """
    Stream one time partition from the DB → PFC archive.
    ...
    try:
        # Client-side cursor with fetchmany() batching.
        # CrateDB does not support server-side named cursors outside transactions.
        # fetchmany(batch_size) is memory-safe: psycopg2 buffers batch_size rows at once.
        cur = conn.cursor()
...
def delete_partition(
    ...
    log.info(f"  Deleting {from_ts.date()} → {to_ts.date()} from {full_tbl} ...")
...
def archive_cycle(cfg: dict, pfc_binary: str, dry_run: bool = False):
    ...
    output_dir      = arch_cfg["output_dir"]
    ...
    for from_ts, to_ts in partitions:
        ...
        pfc_name  = f"{schema}__{table}__{date_str}__{date_end}.pfc"

        # Use local temp dir for the .pfc file before upload
...
def main():
    parser = argparse.ArgumentParser(
        prog="pfc-archiver",
        description="Automated PFC cold-storage archiving daemon for time-series DBs",
    )
    parser.add_argument(
        "--config", "-c",
    ...
    parser.add_argument(
        "--version", action="version", version=f"pfc-archiver {__version__}"
    )
    ...
    log.info(f"pfc-archiver {__version__} starting  |  config: {args.config}")
    log.info(
        f"DB: {cfg['db'].get('host')}:{cfg['db'].get('port', 5432)}"
